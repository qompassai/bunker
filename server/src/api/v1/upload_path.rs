...
use bunker::api::v1::upload_path::{
    UploadPathNarInfo, UploadPathResult, UploadPathResultKind, BUNKER_NAR_INFO,
    BUNKER_NAR_INFO_PREAMBLE_SIZE,
};
use bunker::chunking::chunk_stream;
use bunker::hash::Hash;
use bunker::stream::{read_chunk_async, StreamHasher};
use bunker::util::Finally;
...
use crate::database::{BunkerDatabase, ChunkGuard, NarGuard};
...
pub(crate) async fn upload_path(
    ...
    let upload_info: UploadPathNarInfo = {
        if let Some(preamble_size_bytes) = headers.get(BUNKER_NAR_INFO_PREAMBLE_SIZE) {
        ...
        } else if let Some(nar_info_bytes) = headers.get(BUNKER_NAR_INFO) {
            serde_json::from_slice(nar_info_bytes.as_bytes()).map_err(ServerError::request_error)?
        } else {
            return Err(ErrorKind::RequestError(anyhow!("{} must be set", BUNKER_NAR_INFO)).into());
        }
    };
