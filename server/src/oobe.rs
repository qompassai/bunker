...
//! to let them quickly have a taste of Bunker with a config
//! template that provide guidance for them to achieve a more
//! permanent setup.
//!
//! Paths:
//! - Config: `~/.config/bunker/server.yaml`
//! - SQLite: `~/.local/share/bunker/server.db`
//! - NARs: `~/.local/share/bunker/storage`
...
pub async fn run_oobe() -> Result<()> {
    ...
    eprintln!();
    eprintln!("    bunker login local http://localhost:8080 {root_token}");
    eprintln!();
    eprintln!("Documentations and guides:");
    eprintln!();
    eprintln!("    https://docs.bunker.rs");
    eprintln!();
    eprintln!("Enjoy!");
    eprintln!("-----------------");
    eprintln!();
