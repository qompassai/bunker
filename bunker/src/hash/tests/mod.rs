...
fn test_from_typed() {
    ...
    assert!(matches!(
        ...
        Err(BunkerError::HashError(Error::NoColonSeparator))
    ));
    assert!(matches!(
        Hash::from_typed("sha256:"),
        Err(BunkerError::HashError(
            Error::InvalidHashStringLength { .. }
        ))
    ));
    assert!(matches!(
        Hash::from_typed("sha256:eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee"),
        Err(BunkerError::HashError(Error::InvalidBase32Hash))
    ));
    assert!(matches!(
        Hash::from_typed("sha256:gggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg"),
        Err(BunkerError::HashError(Error::InvalidBase16Hash(_)))
    ));
    assert!(matches!(
        Hash::from_typed("md5:invalid"),
        Err(BunkerError::HashError(Error::UnsupportedHashAlgorithm(alg))) if alg == "md5"
    ));
}
