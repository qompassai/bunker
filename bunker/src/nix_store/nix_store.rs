...
impl NixStore {
    pub fn connect() -> BunkerResult<Self> {
        #[allow(unsafe_code)]
    ...
    /// follows symlinks.
    pub fn follow_store_path<P: AsRef<Path>>(&self, path: P) -> BunkerResult<StorePath> {
    ...
    pub fn parse_store_path<P: AsRef<Path>>(&self, path: P) -> BunkerResult<StorePath> {
    ...
    pub async fn query_path_info(&self, store_path: StorePath) -> BunkerResult<ValidPathInfo> {
        let inner = self.inner.clone();
